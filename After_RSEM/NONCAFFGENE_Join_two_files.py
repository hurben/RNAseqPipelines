...
def files_to_dict(matrix_file, matrix_dict):
...
def intersection_among_files(matrix_1_dict, matrix_2_dict, result_dict):
	...
	for gene in matrix_1_dict.keys():
		...
		if gene in matrix_2_dict.keys():
			...
			fpkm_2 = matrix_2_dict[gene][0]

			status_1 = matrix_1_dict[gene][1]
			status_2 = matrix_2_dict[gene][1]

			if status_1 == status_2:

				result_dict[gene] = [fpkm_1, fpkm_2, status_1, status_2]
...
def result_to_text(result_dict, txt):	
	...
	for genes in result_dict.keys():
		...
		status_1 = result_dict[genes][2]
		status_2 = result_dict[genes][3]

		txt.write(str(genes) + '\t' + str(fpkm_1) + '\t' + str(fpkm_2) + '\t' + str(status_1) + '\t' +  str(status_2) + '\n')
...
if __name__ == '__main__':
	...
	files_to_dict(matrix_1_file, matrix_1_dict)
	files_to_dict(matrix_2_file, matrix_2_dict)
