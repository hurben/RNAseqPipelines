...
for i in range(len(sam_list_readlines)):
	...
	output_name = str(input_file) + 'split.bam'
	...
	cmd = 'java -jar ' + str(gatk_package) +' -T SplitNCigarReads -R ' +str(ref_seq) + ' -I ' +str(read) + ' -o ./' +str(folder_name) +'/' + str(output_name) + ' -rf ReassignOneMappingQuality -RMQF 255 -RMQT 60 -U ALLOW_N_CIGAR_READS'
